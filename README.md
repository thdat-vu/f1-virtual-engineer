...
* **Admin observability** — `/admin` is gated behind `NEXT_PUBLIC_ADMIN_USER_ID` and renders rolling p50/p95/max latency + error rate per route, plus the worker counters (`completed_24h`, `failed_24h`, Redis backend status). Auto-refreshes every 5s.
